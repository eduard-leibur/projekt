...
def lehe_loomine(nimekiri):
    ...
    lisatud_kirje.set("Lisatava mängu pealkiri")
    ...
    nimekirjade_asukoht = "andmed/kasutajad/" + kasutaja + "/mängud.json"
    with open(nimekirjade_asukoht, "r") as nimekirjade_fail:
        nimekirjade_sõnastik = json.load(nimekirjade_fail)

    i = 1
    for mäng in nimekirjade_sõnastik[nimekiri]:
        mäng = str(str(i) + ". " + mäng + "\n")
        nimekirja_kast.insert(tkinter.END, mäng)
        i += 1
