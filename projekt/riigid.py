...
def lehe_loomine(nimekiri):
    ...
    lisatud_kirje.set("Lisatava riigi nimi")
    ...
    nimekirjade_asukoht = "andmed/kasutajad/" + kasutaja + "/riigid.json"
    with open(nimekirjade_asukoht, "r") as nimekirjade_fail:
        nimekirjade_sõnastik = json.load(nimekirjade_fail)

    i = 1
    for riik in nimekirjade_sõnastik[nimekiri]:
        riik = str(str(i) + ". " + riik + "\n")
        nimekirja_kast.insert(tkinter.END, riik)
        i += 1
